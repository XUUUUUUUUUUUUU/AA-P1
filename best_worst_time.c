...
short run_test(pfunc_sort metodo, char *file_name, int num_min, int num_max, int incr)
{
    int i, j, n_times, N;
    ...
    {
        best_case_arr = best_case_array(N);
        worst_case_arr = worst_case_array(N);

        if (best_case_arr == NULL || worst_case_arr == NULL)
        {
